...
public class PostController {
    ...
    public ResponseEntity<List<User>> getVoters(@PathVariable String postId, @PathVariable String option) {
        ...
        Post post = postOptional.get();
        Poll poll = post.getPoll(); // Assuming Post has a Poll object

        if (poll == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
        }

        // Extract voters who voted for the given option
        Map<String, String> voters = poll.getVoters(); // Assuming voters map is <userId, option>
        List<String> voterIds = voters.entrySet().stream()
                .filter(entry -> entry.getValue().equals(option)) // Match the option
                .map(Map.Entry::getKey) // Extract user IDs
                .collect(Collectors.toList());

        // Fetch user details for each voter
        List<User> votersDetails = userRepository.findAllById(voterIds); // Assuming userRepository is available

        return ResponseEntity.ok(votersDetails);
    }
    ...
    public ResponseEntity<List<Map<String, Object>>> getUserPostsWithDetails(@PathVariable String userId) {
        List<Map<String, Object>> posts = postService.getUserPostsWithDetails(userId);
        return ResponseEntity.ok(posts);
    }
    }
